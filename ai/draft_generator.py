...
def generate_draft(
    ...
    **kwargs,
    ...
    # -------------------------------------------------
    # PHASE 1.2 — LLM DRAFT GENERATION (ALWAYS ON)
    # -------------------------------------------------
    system_prompt = (
    "You are a helpful, calm customer support assistant. "
    "Answer the customer's latest message clearly and accurately. "
    ...
    print("🔥 LLM RAW OUTPUT:", repr(draft_text))
    ...
    def _draft_for_intent(intent: str) -> str:
        ...
        # ----------------------------
        # FINAL DRAFT SELECTION
        # ----------------------------
        if draft_text and str(draft_text).strip():
            final_draft = draft_text
        elif template_draft:
            final_draft = template_draft
        else:
            raise RuntimeError("No draft generated by LLM or fallback")
