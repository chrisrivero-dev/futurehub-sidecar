...
from intent_classifier import detect_intent, SAFE_INTENTS, UNSAFE_INTENTS
...
def draft():
    ...
    if (
        ...
        and classification_metadata.get("order_number")
    ):
    ...
    if canned_response_suggestion:
        response["agent_guidance"]["canned_response_suggestion"] = (
...
def build_reason(classification, auto_send_eligible):
    ...
    # Auto-send success case (ONLY shipping_status)
    ...
    # Fallback: safe but not eligible (low confidence, attachments, etc.)
    return (
